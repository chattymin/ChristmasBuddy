...
**Christmas Buddy**は、あなたのMacデスクトップにクリスマスの雰囲気をお届けする可愛いデスクトップコンパニオンアプリです！愛らしいキャラクターがデスクトップに住み、時間帯に合わせたメッセージを届けたり、散らばったギフトボックスを集めたりします。
...
3. DMG内の**Install.command**をダブルクリックしてください（隔離属性を自動削除）
   ```bash
   ```
4. 初回起動時は`システム設定 > プライバシーとセキュリティ`でアプリの実行を許可してください
5. メニューバーの🎄アイコンでキャラクター変更や設定を調整できます
